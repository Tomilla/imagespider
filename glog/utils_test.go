...
func TestCheckPathExists(t *testing.T) {
    assert.Equal(t, CheckPathExists("/tmp"), true)
    assert.Equal(t, CheckPathExists("/etc"), true)
    assert.Equal(t, CheckPathExists("/sjfajute"), false)
    assert.Equal(t, CheckPathExists("/baksejef239u4r"), false)
}
